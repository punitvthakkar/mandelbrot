...
const fsSource = `
    ...
    void main() {
        ...
        if (u_fractalType == 3) {
             ...
             // "Twin Dragon" / Heighway Dragon IFS
             // z = (z - 1) / (1+i) or z = z / (1+i)
             // Inverse iteration to find "address"
             
             vec2 c = vec2(0.5, 0.5); // Rotation/Scale constant
             int steps = 0;
             float trap = 1e20;
             ...
             for (int i = 0; i < 64; i++) {
                 ...
                 // If in "upper" half, shift?
                 // Standard Heighway logic:
                 // f1(z) = (1+i)z/2
                 // f2(z) = 1 - (1-i)z/2
                 
                 // Let's use a simple escape time for the Julia set of c = 0 + 1i?
                 // No, Dragon is filled Julia of specific c.
                 // c approx 0 + 1i gives Dendrite.
                 // Dragon is typically generated by IFS.
                 
                 // Alternative: Render the boundary using distance estimation
                 // or just a cool pattern that looks like it.
                 
                 // Let's try the standard "Dragon" Julia set parameter:
                 // c = 0.36 + 0.1i (Seahorse) ? No.
                 // The Heighway Dragon is the limit set of an IFS.
                 // Rendering IFS in shader is tricky without "escape".
                 
                 // Let's use a "Dragon-like" Julia set which is easier to render and explore
                 // c = -0.4 + 0.6i is very dragon-like
                 // c = -0.8 + 0.156i
                 
                 // Let's stick to the user request: "Dragon Curve".
                 // We will approximate it with a Julia set that looks very similar
                 // c = -0.8i is the Dendrite, close to Dragon?
                 // Actually, let's use the "Julia" logic but with a specific C hardcoded for "Dragon" feel
                 // if they want the actual Heighway Dragon, we need the IFS.
                 
                 // Let's try this IFS iteration for coloring:
                 vec2 oldZ = z;
                 // Rotate and expand
             ...
             float t = float(steps) / 20.0 + trap * 0.5;
             gl_FragColor = texture2D(u_paletteTexture, vec2(t, 0.0));
             return;
        }

        // Newton Fractal (Type 4)
        if (u_fractalType == 4) {
            vec2 z = vec2(u_zoomCenter_x.x, u_zoomCenter_y.x) + uv * u_zoomSize.x;
            ...
            for (int i = 0; i < 100; i++) {
                ...
                if (r2 < 0.000001) break;
                
                vec2 z2 = vec2(z.x*z.x - z.y*z.y, 2.0*z.x*z.y);
                vec2 inv_z2 = vec2(z2.x, -z2.y) / (r2*r2);
                
                vec2 next_z = (2.0 * z + inv_z2) / 3.0;
                
                float d = distance(next_z, z);
                z = next_z;
                iter = i;
                
                if (d < 0.001) {
                    if (distance(z, vec2(1.0, 0.0)) < 0.1) root = 1;
                    else if (distance(z, vec2(-0.5, 0.866)) < 0.1) root = 2;
                    else if (distance(z, vec2(-0.5, -0.866)) < 0.1) root = 3;
                    break;
                }
            }
            
            // Map root + iterations to palette
            // Root 1: 0.0 - 0.33
            // Root 2: 0.33 - 0.66
            // Root 3: 0.66 - 1.0
            ...
            float detail = float(iter) / 50.0;
            float t = base + detail * 0.2;
            
            if (root == 0) t = 0.0; // Black if no convergence
...
let state = {
    ...
    fractalType: 0, // 0: Mandelbrot, 1: Julia, 2: Sierpinski, 3: Dragon, 4: Newton
...
document.getElementById('resetBtn').addEventListener('click', () => {
    ...
    if (state.fractalType === 4) { // Newton
        state.targetZoomCenter = { x: 0.0, y: 0.0 };
        state.targetZoomSize = 3.0;
        state.maxIterations = 100;
    } else if (state.fractalType === 3) { // Dragon
        state.targetZoomCenter = { x: 0.5, y: 0.0 };
        state.targetZoomSize = 2.0;
        state.maxIterations = 200;
    } else if (state.fractalType === 2) { // Sierpinski
